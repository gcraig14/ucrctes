...
if ( ! class_exists('DPF_Admin_Tab') ) {
	...
	class DPF_Admin_Tab {
		...
		public function createOption( $settings ){
			if ( ! apply_filters( 'dpf_create_option_continue_' . $this->owner->owner->optionNamespace, true, $settings ) ) {
				return null;
			}
			...
			do_action( 'dpf_create_option_' . $this->owner->owner->optionNamespace, $obj );

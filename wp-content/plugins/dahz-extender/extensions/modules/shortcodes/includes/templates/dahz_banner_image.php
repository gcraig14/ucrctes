...
# Shortcode class
$shortcode_classes = array( 'de-sc-banner-image uk-position-relative' );
...
# Hover class
$hover_classes = array( 'de-sc-banner-image__image' );
...
# Title attr
$title_attr = array( 'class' => 'uk-margin-small' );

# Title style
$title_style = array();

# Description attr
$description_attr = array( 'class' => array( 'de-sc-banner-image__description uk-width-1-1' ) );
...
# SETTING HOVER IMAGE
	...
	if ( !empty( $hover_image ) ) {
		...
		if ( !empty( $image_size ) && !in_array( $image_size, $defined_image_size ) ) {
			...
			$image_src = wpb_resize( $hover_image, null, $size[0], $size[1], true );

			$bg_url = $image_src['url'];

			$bg_height = ( $image_src['height'] / $image_src['width'] ) * 100;
		} else {
			$image_src = wp_get_attachment_image_src( $hover_image, $image_size, false );
...
# SETTING BUTTON
	...
	# Button attr
	$link_attr['href'] = !empty( $button_link ) ? $button_link : '#';
	...
	if ( !empty( $button_title ) )
		$link_attr['title'] = $button_title;

	# Button target
	if ( !empty( $banner_button_target ) )
		$link_attr['target'] = $banner_button_target;
...
<div <?php dahz_shortcode_set_attributes( $shortcode_attr, 'dahz_banner_image_shortcode' ); ?>>
	<a <?php dahz_shortcode_set_attributes( $link_attr ); ?>>
		<?php if ( $image ) : ?>
		...
		<div class="uk-width-1-1 de-sc-banner-image__content-wrap uk-visible-toggle  uk-padding-remove">
			<div class="<?php echo esc_attr( "{$text_position} {$content_padding}" );?>">
				...
				<?php endif; ?>
				<div <?php dahz_shortcode_set_attributes( $description_attr, 'dahz_banner_image_shortcode' ); ?>>
				<?php if ( !empty( $content ) ) : ?>
					<?php echo wpb_js_remove_wpautop( $content, true );?>
				<?php endif; ?>
				</div>
				<?php if ( $button_visibility !== 'hidden' ) : ?>
					<span <?php dahz_shortcode_set_attributes( $button_attr, 'dahz_banner_image_button_shortcode' ); ?>><?php echo esc_html( $button_text ); ?></span>
				<?php endif; ?>
			</div>
		</div>
	</a>

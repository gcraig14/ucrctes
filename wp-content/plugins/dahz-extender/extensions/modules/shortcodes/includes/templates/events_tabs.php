...
switch( $filter_post_by ){
	...
	case 'categories':
		if( !empty( $events_cat_ids ) ) $query_args['tribe_events_cat'] = $events_cat_ids;
		break;

	case 'post_ids':
		$query_args['post__in'] = array_map( 'sanitize_title', explode( ',', $post_events_ids ) );
		break;
}
	...
	<?php
		...
		foreach( $post_query as $post ){
			...
			dahz_framework_get_template(
				...
				'includes/templates/layouts/events/',
		...
		<div class="uk-flex-first uk-width-1-1 uk-margin uk-margin-remove-top">
			<ul <?php dahz_shortcode_set_attributes( 
				...
				<?php foreach( $collected_categories as $category ): ?>
					...
					<li data-uk-filter-control=".tribe_events_cat-<?php echo esc_attr( $category->slug );?>">

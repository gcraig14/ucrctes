...
if ( !function_exists( 'dahz_framework_skip_link' ) ) {
	function dahz_framework_skip_link() {
		echo '<a class="skip-link screen-reader-text de-skip-link" href="#de-archive-content"> Saltar a contenido </a>';
	}
}
	...
	function dahz_framework_pagination( $pagination_type = "number", $masonry = false, $class="", $is_echo = true ) {
		...
		switch ( $pagination_type ) {
			case 'prev_next':
				...
				$pagination_output .= get_previous_posts_link( '<h6><span data-uk-icon="chevron-left"></span> Anterior </h6>' );

				$pagination_output .= '</li><li>';

				$pagination_output .= get_next_posts_link( '<h6> Siguiente <span data-uk-icon="chevron-right"></span></h6>' );
						...
						<div class="uk-width-expand uk-padding-remove-left">
								...
								<ul class="uk-comment-meta uk-subnav uk-subnav-divider uk-margin-remove-top">
									...
									<?php if ( is_user_logged_in() ) { ?>
										<li>
											<?php edit_comment_link( 'Editar', '<span class="de-comments__edit-btn">', '</span>' ); ?>
										</li>
									<?php }	?>
								</ul>
						</div>
...
if ( !function_exists( 'dahz_framework_render_post_content' ) ) {
	function dahz_framework_render_post_content( $id ) {
		the_content( sprintf(
			'Continuar leyendo<span class="screen-reader-text"> "%s"</span>',
			get_the_title()
		) );
...
if ( !function_exists( 'dahz_framework_render_404_widget' ) ) {
	function dahz_framework_render_404_widget() {
		...
		$archive_content = '<p>' . sprintf( 'Intenta buscar en los archivos mensuales. %1$s', convert_smilies( ':)' ) ) . '</p>';

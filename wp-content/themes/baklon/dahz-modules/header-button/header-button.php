	...
	Class Dahz_Framework_Header_Button {
		...
		public function dahz_framework_header_button_init( $path ){
			...
			dahz_framework_register_customizer(
				...
				array(
					'id'	=> 'header_button',
					'title'	=> array( 'title' => esc_html__( 'Button Header', 'baklon' ), 'priority' => 19 ),
					'panel'	=> 'header'
				),
				array()
			);
		...
		public function dahz_framework_header_item_button( $items ) {
			...
			$items['button'] = array(
				'title'				=> esc_html__( 'Button', 'baklon' ),
				'description'		=> esc_html__( 'Display Button in Header', 'baklon' ),
		...
		public function dahz_framework_header_button_element( $builder_type, $section, $row, $column ){
			...
			echo sprintf(
				'
				<a class="de-header-button uk-button uk-button-default" target="%2$s">%1$s</a>
				',
